...
class Qwen_MMDiT(baseframework):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "florence" not in self.config.framework.qwenvl.base_vlm.lower():
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            attention_mask_repeated = attention_mask.to(last_hidden.device, dtype=last_hidden.dtype).repeat(repeated_diffusion_steps, 1) if attention_mask is not None else None
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        attention_mask = attention_mask.to(last_hidden.device, dtype=last_hidden.dtype) if attention_mask is not None else None
