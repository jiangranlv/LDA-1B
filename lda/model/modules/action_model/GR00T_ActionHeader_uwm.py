...
from LDA.model.modules.action_model.flow_matching_head.cdit_uwm import CDiT as DiT
    ...
    def predict_action(
        ...
        if self.use_img_denoise:
            if self.vision_encoder_type == "vjepa2":
                ...
                obs_shape = next_obs_sample.shape[:-2]  # (B, V, C, T)
            elif self.vision_encoder_type == "vae":
        ...
        for t in range(num_steps):
            ...
            if self.config.add_pos_embed:
                ...
                pos_embs = None
            # Join vision, language, state and action embedding along sequence dimension (same as forward)
            if self.use_img_denoise:
                sa_embs = torch.cat([future_tokens, action_features, noisy_next_obs], dim=1)
            else:
                sa_embs = torch.cat([future_tokens, action_features], dim=1)
